...
С 2026-08-18 Aeza используется как **entry relay** и как jump-хост к 104:
- клиент из внешней сети подключается к `203.0.113.20:443` по Reality+XHTTP;
...
- Тестовый хост — любой посторонний VPS. Aeza `203.0.113.20` теперь является рабочим entry relay, поэтому она не независима для теста полной цепочки через 178; её допустимо использовать только для точечных проверок downstream 104/130, не меняя production Xray.
