...
### 2026-08-28
- **Профиль `WSL → 178 → 104 → 130` снова включён как штатный вариант.** Selector сохраняет выбор между двумя маршрутами, а единый fail-closed killswitch переживает cold boot и восстановление. Для SSH к entry прямое исключение ограничено только транспортом Xray `TCP/443`; `TCP/22` идёт через TUN. Из sing-box удалено старое destination-wide правило `direct`, которое обходило внутреннюю цепочку. Проверены фактический SSH-вход и выход через VPN. Подробности — [`wsl_178_104_130/TEST_RESULTS.md`](./wsl_178_104_130/TEST_RESULTS.md).

### 2026-08-20
- **Добавлена ресурсная защита entry/exit nodes.** На entry создан swap 1 ГБ и journal ограничен 300 МБ; на primary exit Xray переведён `debug → warning`, добавлена ежедневная ротация access-лога; на secondary exit добавлены ротация Xray/btmp и лимит journal 50 МБ. Автоочистка выполняется штатным `logrotate.timer` без постоянной нагрузки на RAM. Runbook и проверочные команды — [`OPERATIONS.md`](./OPERATIONS.md) §6.
