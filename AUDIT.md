...
**Текущее состояние WSL (2026-09-12):** холодный старт не включает VPN и оставляет IPv4/IPv6 policies `ACCEPT`; прежние boot/cron launchers VLESS отключены. Владелец вручную выбирает `vless-wsl use 104-130`, `vless-wsl use 178-104-130` или отдельный OpenVPN-over-SSH. Только выбранный launcher применяет свой fail-closed killswitch, и одновременно работает один TUN. Профиль через 178, TLS ClientHello fragment `50-100` / `1-3`, supervisor и `/usr/local/bin/killswitch-vless-178` сохранены как canary/резерв. Кандидатный killswitch разрешает `eth0` только к `203.0.113.20:443` и блокирует IPv6. Расширенный тест ранее выявил отдельные timeout, обрыв длинной загрузки и более сложное восстановление после рестарта 178.
